...
SUBROUTINE cloudsc(kidia, kfdia, klon, klev, ptsphy_var_5, pt_var_6, pq_var_7, ptendency_tmp_t, ptendency_tmp_q, ptendency_tmp_a, ptendency_tmp_cld, ptendency_loc_t, ptendency_loc_q, ptendency_loc_a, ptendency_loc_cld, pvfa_var_8, pvfl_var_9, pvfi_var_10, pdyna_var_11, pdynl_var_12, pdyni_var_13, phrsw_var_14, phrlw_var_15, pvervel_var_16, pap_var_17, paph_var_18, plsm_var_19, ldcum_var_20, ktype_var_21, plu_var_22, plude_var_23, psnde_var_24, pmfu_var_25, pmfd_var_26, pa_var_27, pclv_var_29, psupsat_var_30, plcrit_aer_var_0, picrit_aer_var_1, pre_ice_var_2, pccn_var_3, pnice_var_4, pcovptot_var_31, prainfrac_toprfz_var_32, pfsqlf_var_33, pfsqif_var_34, pfcqnng_var_36, pfcqlng_var_35, pfsqrf_var_37, pfsqsf_var_38, pfcqrng_var_39, pfcqsng_var_40, pfsqltur_var_41, pfsqitur_var_42, pfplsl_var_43, pfplsn_var_44, pfhpsl_var_45, pfhpsn_var_46, kfldx_var_28, ydcst, ydthf, ydecldp_var_47)
  ...
  TYPE(tecldp), INTENT(IN) :: ydecldp_var_47
  ...
  zcons1a = ydcst % rcpd / (ydcst % rlmlt * ydcst % rg * ydecldp_var_47 % rtaumel)
  zepsec = 1D-14
  ...
  zvqx(5) = 0.0D0
  zvqx(1) = 0.0D0
  zvqx(2) = ydecldp_var_47 % rvice
  zvqx(3) = ydecldp_var_47 % rvrain
  zvqx(4) = ydecldp_var_47 % rvsnow
  llfall(:) = .FALSE.
  DO jm = 1, 5
  ...
  DO jk = 1, klev
    DO jl = 1, kfdia
      IF (zqx(jl, jk, 1) + zqx(jl, jk, 2) < ydecldp_var_47 % rlmin .OR. za(jl, jk) < ydecldp_var_47 % ramin) THEN
  ...
  DO jm = 1, 4
    DO jk = 1, klev
      DO jl = 1, kfdia
        IF (zqx(jl, jk, jm) < ydecldp_var_47 % rlmin) THEN
  ...
  DO jk = 1, klev
    DO jl = 1, kfdia
      ...
      oka = ztp1(jl, jk)
      zfoeew(jl, jk) = MIN((zalfa * ydthf % r2es * EXP(ydthf % r3les * (oka - ydcst % rtt) / (oka - ydthf % r4les)) + (1.0D0 - zalfa) * ydthf % r2es * EXP(ydthf % r3ies * (oka - ydcst % rtt) / (oka - ydthf % r4ies))) / pap_var_17(jl, jk), 0.5D0)
      zfoeew(jl, jk) = MIN(0.5D0, zfoeew(jl, jk))
      zqsice(jl, jk) = zfoeew(jl, jk) / (1.0D0 - ydcst % retv * zfoeew(jl, jk))
      oka = ztp1(jl, jk)
      zfoeeliqt(jl, jk) = MIN(ydthf % r2es * EXP(ydthf % r3les * (oka - ydcst % rtt) / (oka - ydthf % r4les)) / pap_var_17(jl, jk), 0.5D0)
  ...
  DO jk = 1, klev
    DO jl = 1, kfdia
      ...
      IF (zli(jl, jk) > ydecldp_var_47 % rlmin) THEN
  ...
  zcovptot(:) = 0.0D0
  zcldtopdist(:) = 0.0D0
  DO jk = ydecldp_var_47 % ncldtop, klev
    DO jm = 1, 5
      DO jl = 1, kfdia
    ...
    DO jl = 1, kfdia
      ...
      zrho(jl) = pap_var_17(jl, jk) / (ydcst % rd * ztp1(jl, jk))
      zdtgdp(jl) = ptsphy_var_5 * zgdp(jl)
      zrdtgdp(jl) = zdp(jl) * (1.0D0 / (ptsphy_var_5 * ydcst % rg))
      IF (jk > 1) zdtgdpf(jl) = ptsphy_var_5 * ydcst % rg / (pap_var_17(jl, jk) - pap_var_17(jl, jk - 1))
    ...
    END DO
    DO jl = 1, kfdia
      IF (zqx(jl, jk, 1) < ydecldp_var_47 % rlmin) THEN
        zsolqa(jl, 5, 1) = zqx(jl, jk, 1)
        zsolqa(jl, 1, 5) = - zqx(jl, jk, 1)
      END IF
      IF (zqx(jl, jk, 2) < ydecldp_var_47 % rlmin) THEN
    ...
    END DO
    DO jl = 1, kfdia
      IF (ztp1(jl, jk) >= ydcst % rtt .OR. ydecldp_var_47 % nssopt == 0) THEN
        zfac = 1.0D0
        zfaci = 1.0D0
      ELSE
        zfac = za(jl, jk) + zfokoop(jl) * (1.0D0 - za(jl, jk))
        zfaci = ptsphy_var_5 / ydecldp_var_47 % rkooptau
      END IF
      IF (za(jl, jk) > 1.0D0 - ydecldp_var_47 % ramin) THEN
      ...
      IF (zsupsat(jl) > 1D-14) THEN
        IF (ztp1(jl, jk) > ydecldp_var_47 % rthomo) THEN
      ...
      END IF
    END DO
    IF (jk < klev .AND. jk >= ydecldp_var_47 % ncldtop) THEN
      DO jl = 1, kfdia
        plude_var_23(jl, jk) = plude_var_23(jl, jk) * zdtgdp(jl)
        IF (ldcum_var_20(jl) .AND. plude_var_23(jl, jk) > ydecldp_var_47 % rlmin .AND. plu_var_22(jl, jk + 1) > 1D-14) THEN
      ...
      END DO
    END IF
    IF (jk > ydecldp_var_47 % ncldtop) THEN
      DO jl = 1, kfdia
    ...
    END DO
    DO jl = 1, kfdia
      zldifdt(jl) = ydecldp_var_47 % rcldiff * ptsphy_var_5
      IF (ktype_var_21(jl) > 0 .AND. plude_var_23(jl, jk) > 1D-14) zldifdt(jl) = ydecldp_var_47 % rcldiff_convi * zldifdt(jl)
    END DO
    DO jl = 1, kfdia
    ...
    DO jl = 1, kfdia
      ...
      zwtot = pvervel_var_16(jl, jk) + 0.5D0 * ydcst % rg * (pmfu_var_25(jl, jk) + pmfd_var_26(jl, jk) + zmfdn)
      zwtot = MIN(zdpmxdt, MAX(- zdpmxdt, zwtot))
      zzzdt = phrsw_var_14(jl, jk) + phrlw_var_15(jl, jk)
      zdtdiab = MIN(zdpmxdt * zdtdp, MAX(- zdpmxdt * zdtdp, zzzdt)) * ptsphy_var_5 + ydthf % ralfdcp * zldefr(jl)
    ...
    END DO
    DO jl = 1, kfdia
      IF (za(jl, jk) > 1D-14 .AND. zdqs(jl) <= - ydecldp_var_47 % rlmin) THEN
        ...
        IF (zlcond1(jl) < ydecldp_var_47 % rlmin) zlcond1(jl) = 0.0D0
        IF (ztp1(jl, jk) > ydecldp_var_47 % rthomo) THEN
    ...
    END DO
    DO jl = 1, kfdia
      IF (zdqs(jl) <= - ydecldp_var_47 % rlmin .AND. za(jl, jk) < 0.99999999999999D0) THEN
        zrhc = ydecldp_var_47 % ramid
        zsigk = pap_var_17(jl, jk) / paph_var_18(jl, klev + 1)
        IF (zsigk > 0.8D0) THEN
          zrhc = ydecldp_var_47 % ramid + (1.0D0 - ydecldp_var_47 % ramid) * ((zsigk - 0.8D0) / 0.2D0) ** 2
        END IF
        IF (ydecldp_var_47 % nssopt == 0) THEN
          zqe = (zqx(jl, jk, 5) - za(jl, jk) * zqsice(jl, jk)) / MAX(zepsec, 1.0D0 - za(jl, jk))
          zqe = MAX(0.0D0, zqe)
        ELSE IF (ydecldp_var_47 % nssopt == 1) THEN
          zqe = (zqx(jl, jk, 5) - za(jl, jk) * zqsice(jl, jk)) / MAX(zepsec, 1.0D0 - za(jl, jk))
          zqe = MAX(0.0D0, zqe)
        ELSE IF (ydecldp_var_47 % nssopt == 2) THEN
          zqe = zqx(jl, jk, 5)
        ELSE IF (ydecldp_var_47 % nssopt == 3) THEN
        ...
        IF (zqe >= zrhc * zqsice(jl, jk) * zfac .AND. zqe < zqsice(jl, jk) * zfac) THEN
          ...
          IF (zlcond2(jl) < ydecldp_var_47 % rlmin .OR. (1.0D0 - za(jl, jk)) < 1D-14) THEN
            zlcond2(jl) = 0.0D0
            zacond = 0.0D0
          END IF
    ...
    END DO
    DO jl = 1, kfdia
      IF (za(jl, jk - 1) < ydecldp_var_47 % rcldtopcf .AND. za(jl, jk) >= ydecldp_var_47 % rcldtopcf) THEN
        zcldtopdist(jl) = 0.0D0
      ELSE
        zcldtopdist(jl) = zcldtopdist(jl) + zdp(jl) / (zrho(jl) * ydcst % rg)
      END IF
      IF (ztp1(jl, jk) < ydcst % rtt .AND. zqxfg(jl, 1) > ydecldp_var_47 % rlmin) THEN
        oka = ztp1(jl, jk)
        ...
        zbdd = ydcst % rv * ztp1(jl, jk) * pap_var_17(jl, jk) / (2.21D0 * zvpice)
        zcvds = 7.8D0 * (zicenuclei(jl) / zrho(jl)) ** 0.666D0 * (zvpliq - zvpice) / (8.87D0 * (zadd + zbdd) * zvpice)
        zice0 = MAX(zicecld(jl), zicenuclei(jl) * ydecldp_var_47 % riceinit / zrho(jl))
        ...
        zdepos = zdepos * MIN(zinfactor + (1.0D0 - zinfactor) * (ydecldp_var_47 % rdepliqrefrate + zcldtopdist(jl) / ydecldp_var_47 % rdepliqrefdepth), 1.0D0)
    ...
    DO jm = 1, 5
      IF (llfall(jm) .OR. jm == 2) THEN
        DO jl = 1, kfdia
          ...
          END IF
          IF (ydecldp_var_47 % laericesed .AND. jm == 2) THEN
    ...
    DO jl = 1, kfdia
      IF (zqpretot(jl) > 1D-14) THEN
        ...
        zcovptot(jl) = MAX(zcovptot(jl), ydecldp_var_47 % rcovpmin)
    ...
    DO jl = 1, kfdia
      IF (ztp1(jl, jk) <= ydcst % rtt) THEN
        IF (zicecld(jl) > 1D-14) THEN
          zzco = ptsphy_var_5 * ydecldp_var_47 % rsnowlin1 * EXP(ydecldp_var_47 % rsnowlin2 * (ztp1(jl, jk) - ydcst % rtt))
          IF (ydecldp_var_47 % laericeauto) THEN
            zlcrit = picrit_aer_var_1(jl, jk)
            zzco = zzco * (ydecldp_var_47 % rnice / pnice_var_4(jl, jk)) ** 0.333D0
          ELSE
            zlcrit = ydecldp_var_47 % rlcritsnow
          END IF
      ...
      IF (zliqcld(jl) > 1D-14) THEN
        IF (plsm_var_19(jl) > 0.5D0) THEN
          zconst = ydecldp_var_47 % rcl_kk_cloud_num_land
          zlcrit = ydecldp_var_47 % rclcrit_land
        ELSE
          zconst = ydecldp_var_47 % rcl_kk_cloud_num_sea
          zlcrit = ydecldp_var_47 % rclcrit_sea
        END IF
        IF (zliqcld(jl) > zlcrit) THEN
          zrainaut(jl) = 1.5D0 * za(jl, jk) * ptsphy_var_5 * ydecldp_var_47 % rcl_kkaau * zliqcld(jl) ** ydecldp_var_47 % rcl_kkbauq * zconst ** ydecldp_var_47 % rcl_kkbaun
          zrainaut(jl) = MIN(zrainaut(jl), zqxfg(jl, 1))
          IF (zrainaut(jl) < 1D-14) zrainaut(jl) = 0.0D0
          zrainacc(jl) = 2.0D0 * za(jl, jk) * ptsphy_var_5 * ydecldp_var_47 % rcl_kkaac * (zliqcld(jl) * zraincld(jl)) ** ydecldp_var_47 % rcl_kkbac
    ...
    DO jl = 1, kfdia
      IF (ztp1(jl, jk) <= ydcst % rtt .AND. zliqcld(jl) > 1D-14) THEN
        zfallcorr = (ydecldp_var_47 % rdensref / zrho(jl)) ** 0.4D0
        IF (zsnowcld(jl) > 1D-14 .AND. zcovptot(jl) > 0.01D0) THEN
          zsnowrime(jl) = 0.3D0 * zcovptot(jl) * ptsphy_var_5 * ydecldp_var_47 % rcl_const7s * zfallcorr * (zrho(jl) * zsnowcld(jl) * ydecldp_var_47 % rcl_const1s) ** ydecldp_var_47 % rcl_const8s
    ...
    DO jl = 1, kfdia
      ...
      IF (zicetot(jl) > 1D-14 .AND. ztp1(jl, jk) > ydcst % rtt) THEN
        ...
        ztdmtw0 = ztp1(jl, jk) - ydcst % rtt - zsubsat * (1329.31D0 + 0.0074615D0 * (pap_var_17(jl, jk) - 85000.0D0) - 40.637D0 * (ztp1(jl, jk) - 275.0D0))
        zcons1 = ABS(ptsphy_var_5 * (1.0D0 + 0.5D0 * ztdmtw0) / ydecldp_var_47 % rtaumel)
      ...
      IF (zqx(jl, jk, 3) > 1D-14) THEN
        ...
        IF (ztp1(jl, jk) < ydcst % rtt) THEN
          IF (llrainliq(jl)) THEN
            zlambda = (ydecldp_var_47 % rcl_fac1 / (zrho(jl) * zqx(jl, jk, 3))) ** ydecldp_var_47 % rcl_fac2
            ztemp = ydecldp_var_47 % rcl_fzrab * (ztp1(jl, jk) - ydcst % rtt)
            zfrz = ptsphy_var_5 * (ydecldp_var_47 % rcl_const5r / zrho(jl)) * (EXP(ztemp) - 1.0D0) * zlambda ** ydecldp_var_47 % rcl_const6r
            zfrzmax(jl) = MAX(zfrz, 0.0D0)
          ELSE
            zcons1 = ABS(ptsphy_var_5 * (1.0D0 + 0.5D0 * (ydcst % rtt - ztp1(jl, jk))) / ydecldp_var_47 % rtaumel)
    ...
    END DO
    DO jl = 1, kfdia
      zfrzmax(jl) = MAX((ydecldp_var_47 % rthomo - ztp1(jl, jk)) * zrldcp, 0.0D0)
    END DO
    jm = 1
    jnn = imelt(jm)
    DO jl = 1, kfdia
    ...
    END DO
    DO jl = 1, kfdia
      zzrh = ydecldp_var_47 % rprecrhmax + (1.0D0 - ydecldp_var_47 % rprecrhmax) * zcovpmax(jl) / MAX(zepsec, 1.0D0 - za(jl, jk))
      zzrh = MIN(MAX(zzrh, ydecldp_var_47 % rprecrhmax), 1.0D0)
      zzrh = MIN(0.8D0, zzrh)
      ...
      IF (llo1) THEN
        ...
        zlambda = (ydecldp_var_47 % rcl_fac1 / (zrho(jl) * zpreclr)) ** ydecldp_var_47 % rcl_fac2
        zevap_denom = ydecldp_var_47 % rcl_cdenom1 * zesatliq - ydecldp_var_47 % rcl_cdenom2 * ztp1(jl, jk) * zesatliq + ydecldp_var_47 % rcl_cdenom3 * ztp1(jl, jk) ** 3.0D0 * pap_var_17(jl, jk)
        zcorr2 = (ztp1(jl, jk) / 273.0D0) ** 1.5D0 * 393.0D0 / (ztp1(jl, jk) + 120.0D0)
        zka = ydecldp_var_47 % rcl_ka273 * zcorr2
        zsubsat = MAX(zzrh * zqsliq(jl, jk) - zqe, 0.0D0)
        zbeta = (0.5D0 / zqsliq(jl, jk)) * ztp1(jl, jk) ** 2.0D0 * zesatliq * ydecldp_var_47 % rcl_const1r * (zcorr2 / zevap_denom) * (0.78D0 / (zlambda ** ydecldp_var_47 % rcl_const4r) + ydecldp_var_47 % rcl_const2r * (zrho(jl) * zfallcorr) ** 0.5D0 / (zcorr2 ** 0.5D0 * zlambda ** ydecldp_var_47 % rcl_const3r))
        ...
        zcovptot(jl) = MAX(ydecldp_var_47 % rcovpmin, zcovptot(jl) - MAX(0.0D0, (zcovptot(jl) - za(jl, jk)) * zevap / zqxfg(jl, 3)))
    ...
    DO jl = 1, kfdia
      ...
      IF (llo1) THEN
        ...
        zbeta1 = SQRT(pap_var_17(jl, jk) / paph_var_18(jl, klev + 1)) / ydecldp_var_47 % rvrfactor * zpreclr / MAX(zcovpclr(jl), zepsec)
        zbeta = ydcst % rg * ydecldp_var_47 % rpecons * (zbeta1) ** 0.5777D0
        ...
        zcovptot(jl) = MAX(ydecldp_var_47 % rcovpmin, zcovptot(jl) - MAX(0.0D0, (zcovptot(jl) - za(jl, jk)) * zevap / zqxfg(jl, 4)))
    ...
    DO jm = 1, 5
      IF (llfall(jm)) THEN
        DO jl = 1, kfdia
          IF (zqxfg(jl, jm) < ydecldp_var_47 % rlmin) THEN
    ...
    DO jl = 1, kfdia
      ...
      IF (zanew < ydecldp_var_47 % ramin) zanew = 0.0D0
  ...
  DO jk = 1, klev + 1
    DO jl = 1, kfdia
      pfhpsl_var_45(jl, jk) = - ydcst % rlvtt * pfplsl_var_43(jl, jk)
      pfhpsn_var_46(jl, jk) = - ydcst % rlstt * pfplsn_var_44(jl, jk)
    END DO
  END DO
