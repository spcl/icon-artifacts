...
use ti_solve_nh
use serde_solve_nh
use fake_mo_solve_nonhydro, only: solve_nh_fake => solve_nh
...
!   call type_inject(at("p_nh.ti", asis=.true.), p_nh)
!   call type_inject(at("p_nh_diag.ti", asis=.true.), p_nh%diag)
!   call type_inject(at("p_nh_prog_1.ti", asis=.true.), p_nh%prog(1))
!   call type_inject(at("p_nh_prog_2.ti", asis=.true.), p_nh%prog(2))
!   call type_inject(at("p_nh_ref.ti", asis=.true.), p_nh%ref)
!   call type_inject(at("p_nh_metrics.ti", asis=.true.), p_nh%metrics)
!   call type_inject(at("p_patch.ti", asis=.true.), p_patch)
!   call type_inject(at("p_patch_geometry_info.ti", asis=.true.), p_patch%geometry_info)
!   call type_inject(at("p_patch_cells.ti", asis=.true.), p_patch%cells)
!   call type_inject(at("p_patch_edges.ti", asis=.true.), p_patch%edges)
!   call type_inject(at("p_patch_verts.ti", asis=.true.), p_patch%verts)
!   call type_inject(at("p_int.ti", asis=.true.), p_int)
!   call type_inject(at("prep_adv.ti", asis=.true.), prep_adv)
! end if

! if (generation == 9) then
!   call serialize(at("p_int"), p_int)
!   call serialize(at("nnow"), nnow)
!   call serialize(at("nnew"), nnew)
!   call serialize(at("l_init"), l_init)
!   call serialize(at("l_recompute"), l_recompute)
!   call serialize(at("lsave_mflx"), lsave_mflx)
!   call serialize(at("lprep_adv"), lprep_adv)
!   call serialize(at("lclean_mflx"), lclean_mflx)
!   call serialize(at("idyn_timestep"), idyn_timestep)
!   call serialize(at("jstep"), jstep)
!   call serialize(at("dtime"), dtime)
!   call serialize(at("lacc"), lacc)

!   call serialize_global_data(at("global_data.t0"))
!   call serialize(at("p_nh.t0"), p_nh)
!   call serialize(at("p_patch.t0"), p_patch)
!   call serialize(at("prep_adv.t0"), prep_adv)
! end if

! print *, "calling into `solve_nh_fake`"
! call solve_nh_fake (p_nh, p_patch, p_int, prep_adv, nnow, nnew, l_init, l_recompute, lsave_mflx, lprep_adv, lclean_mflx, idyn_timestep, jstep, dtime, lacc)

! if (generation == 9) then
!   call serialize_global_data(at("global_data.t1"))
!   call serialize(at("p_nh.t1"), p_nh)
!   call serialize(at("p_patch.t1"), p_patch)
!   call serialize(at("prep_adv.t1"), prep_adv)
! end if
