...
void deserialize(global_data_type *x, std::istream &s) {
  ...
  read_line(
      s, {"# process_mpi_all_size"}); // Should contain '# process_mpi_all_size'
  ...
  read_line(s, {"# ndyn_substeps_var"}); // Should contain '# ndyn_substeps_var'
...
std::string serialize(const t_nh_metrics *x) {
  ...
  if (x->bdy_halo_c_idx)
    add_line(serialize_array(x->bdy_halo_c_idx), s);
  ...
  if (x->bdy_halo_c_blk)
    add_line(serialize_array(x->bdy_halo_c_blk), s);
  ...
  if (x->mask_prog_halo_c)
    add_line(serialize_array(x->mask_prog_halo_c), s);
