// SOURCE: https://github.com/sailormoon/flags
...
get_value(const argument_map &options, const std::string_view &option) {
...
inline std::vector<std::optional<std::string_view>>
get_values(const argument_map &options, const std::string_view &option) {
  if (const auto it = options.find(option); it != options.end()) {
    return it->second;
  }
  return {};
...
template <>
inline std::vector<std::optional<std::string_view>>
get_multiple(const argument_map &options, const std::string_view &option) {
...
// Special case for booleans: if the value is in the falsities array (see
// get<bool>) the option will be considered falsy. Otherwise, it will be
// considered truthy just for being present.
template <>
inline std::vector<std::optional<bool>>
get_multiple(const argument_map &options, const std::string_view &option) {
  const auto views = get_values(options, option);
  ...
  for (const auto view : views) {
    ...
    values.push_back(
        std::none_of(falsities.begin(), falsities.end(),
...
template <>
inline std::optional<std::string_view>
get(const std::vector<std::string_view> &positional_arguments,
    size_t positional_index) {
  if (positional_index < positional_arguments.size()) {
...
struct args {
  ...
  get_multiple(const std::string_view &option) const {
    return detail::get_multiple<T>(parser_.options(), option);
  }

  template <class T>
  std::vector<T> get_multiple(const std::string_view &option,
  ...
  template <class T> std::optional<T> get(size_t positional_index) const {
    return detail::get<T>(parser_.positional_arguments(), positional_index);
  }

  template <class T> T get(size_t positional_index, T &&default_value) const {
