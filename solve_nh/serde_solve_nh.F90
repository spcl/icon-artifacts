MODULE serde_solve_nh
  ...
  SUBROUTINE W_t_nh_metrics(io, x, cleanup, nline)
    ...
    END IF
    CALL serialize(io, '# bdy_halo_c_idx', cleanup = .FALSE.)
    CALL serialize(io, '# assoc', cleanup = .FALSE.)
    CALL serialize(io, ASSOCIATED(x % bdy_halo_c_idx), cleanup = .FALSE.)
    IF (ASSOCIATED(x % bdy_halo_c_idx)) THEN
      kmeta = 0
      CALL serialize(io, "# missing", cleanup = .FALSE.)
      CALL serialize(io, (kmeta == 0), cleanup = .FALSE.)
      CALL serialize(io, x % bdy_halo_c_idx, cleanup = .FALSE.)
    END IF
    CALL serialize(io, '# bdy_halo_c_blk', cleanup = .FALSE.)
    CALL serialize(io, '# assoc', cleanup = .FALSE.)
    CALL serialize(io, ASSOCIATED(x % bdy_halo_c_blk), cleanup = .FALSE.)
    IF (ASSOCIATED(x % bdy_halo_c_blk)) THEN
      kmeta = 0
      CALL serialize(io, "# missing", cleanup = .FALSE.)
      CALL serialize(io, (kmeta == 0), cleanup = .FALSE.)
      CALL serialize(io, x % bdy_halo_c_blk, cleanup = .FALSE.)
    END IF
    ...
    END IF
    CALL serialize(io, '# mask_prog_halo_c', cleanup = .FALSE.)
    CALL serialize(io, '# assoc', cleanup = .FALSE.)
    CALL serialize(io, ASSOCIATED(x % mask_prog_halo_c), cleanup = .FALSE.)
    IF (ASSOCIATED(x % mask_prog_halo_c)) THEN
      kmeta = 0
      CALL serialize(io, "# missing", cleanup = .FALSE.)
      CALL serialize(io, (kmeta == 0), cleanup = .FALSE.)
      CALL serialize(io, x % mask_prog_halo_c, cleanup = .FALSE.)
    END IF
    IF (nline_local) WRITE(io, '(g0)', ADVANCE = 'no') NEW_LINE('A')
    IF (cleanup_local) CLOSE(UNIT = io)
