...
for got, want in zip(got_files, want_files):
    ...
    with open(got, "r") as got_file, open(want, "r") as want_file:
        got_lines = [l.strip() for l in got_file.readlines()]
        want_lines = [l.strip() for l in want_file.readlines()]
        ...
        for got_line, want_line in zip(got_lines, want_lines):
            ...
            try:
                ...
                if not shutup and (rel_err > 1e-12 or abs_err > 1e-12):
                    print(f"{got} and {want} have numerical differences at {diffcomp} ❌")
                    found_diff = True
                    shutup = True

            except ValueError:
                if got_line not in {'# assoc', '# rank', '# size', '# lbound', '# entries'}:
                    diffcomp = got_line
                    shutup = False
