...
def get_nsdfg_nestedness(sdfg: dace.SDFG) -> typing.List[typing.Tuple[dace.nodes.NestedSDFG, dace.SDFGState, int]]:
    nestedness_list = get_nsdfg_nestedness_impl(sdfg, 1)
    sorted_list = sorted(nestedness_list, key=lambda x: x[2], reverse=True)
    return sorted_list


def get_nsdfg_nestedness_impl(sdfg: dace.SDFG, num_nestedness: int) -> typing.List[typing.Tuple[dace.nodes.NestedSDFG, dace.SDFGState, int]]:
    # Get current NSDFGs
    result = []
    for s in sdfg.all_states():
        for n in s.nodes():
            if isinstance(n, dace.nodes.NestedSDFG):
                result.append((n, s, num_nestedness))
...
def clean_unused_data_from_nsdfg(sdfg: dace.SDFG):
    ...
    for nsdfg_node, nsdfg_state, nestedness in nestedness_list:
        ...
        for data_name in unused_data:
            ...
            if inner_sdfg.arrays[data_name].transient:
