...
void deserialize(gas_type* x, std::istream& s) {
  ...
  x->mixing_ratio =
      m.read<std::remove_pointer<decltype(x->mixing_ratio)>::type>(s);
}
...
void deserialize(config_type* x, std::istream& s) {
  ...
  // interpret (assuming it follows the same protocol as us).
  x->i_band_from_g_lw =
      m.read<std::remove_pointer<decltype(x->i_band_from_g_lw)>::type>(s);
  ...
  // We only need to allocate a volume of contiguous memory, and let DaCe
  // interpret (assuming it follows the same protocol as us).
  x->i_band_from_reordered_g_sw = m.read<
      std::remove_pointer<decltype(x->i_band_from_reordered_g_sw)>::type>(s);
}
...
void deserialize(thermodynamics_type* x, std::istream& s) {
  ...
  // interpret (assuming it follows the same protocol as us).
  x->temperature_fl =
      m.read<std::remove_pointer<decltype(x->temperature_fl)>::type>(s);
  ...
  // interpret (assuming it follows the same protocol as us).
  x->pressure_hl =
      m.read<std::remove_pointer<decltype(x->pressure_hl)>::type>(s);
  ...
  // interpret (assuming it follows the same protocol as us).
  x->temperature_hl =
      m.read<std::remove_pointer<decltype(x->temperature_hl)>::type>(s);
  ...
  // interpret (assuming it follows the same protocol as us).
  x->pressure_fl =
      m.read<std::remove_pointer<decltype(x->pressure_fl)>::type>(s);
}
...
void deserialize(single_level_type* x, std::istream& s) {
  ...
  // interpret (assuming it follows the same protocol as us).
  x->skin_temperature =
      m.read<std::remove_pointer<decltype(x->skin_temperature)>::type>(s);

  read_line(s, {"# spectral_solar_scaling"});  // Should contain '#
                                               // spectral_solar_scaling'

  m = read_array_meta(s);

  // We only need to allocate a volume of contiguous memory, and let DaCe
  // interpret (assuming it follows the same protocol as us).
  x->spectral_solar_scaling =
      m.read<std::remove_pointer<decltype(x->spectral_solar_scaling)>::type>(s);
