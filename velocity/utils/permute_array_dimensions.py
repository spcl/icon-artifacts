...
def _get_fortran_strides(array: dace.data.Array) -> List[int]:
    accum = 1
    strides = []
...
def _get_c_strides(array: dace.data.Array) -> List[int]:
    accum = 1
    strides = []
...
def _is_fortran_strides(array: dace.data.Array) -> bool:
    strides = _get_fortran_strides(array)
    return tuple(strides) == tuple(array.strides)

def _is_c_strides(array: dace.data.Array) -> bool:
    strides = _get_c_strides(array)
    return tuple(strides) == tuple(array.strides)


def inverse_strides(sdfg: dace.SDFG, array_name: str):
    array_desc = sdfg.arrays.get(array_name)
    strides = array_desc.strides

    if _is_fortran_strides(array_desc):
        print(f"Strides for {array_name} is Fortran-style (1->max stride)")
        strides = _get_c_strides(array_desc)
    elif _is_c_strides(array_desc):
        print(f"Strides for {array_name} is C-style (max->1 stride)")
        strides = _get_fortran_strides(array_desc)
    else:
        raise ValueError(f"Strides for {array_name} are neither Fortran nor C-style: {strides}")

    array_desc.strides = tuple(strides)

    for state in sdfg.all_states():
        for node in state.nodes():
            if isinstance(node, dace.nodes.NestedSDFG):
                # If the nested SDFG has the same array, we need to inverse the strides there as well
                if array_name in node.sdfg.arrays:
                    inverse_strides(node.sdfg, array_name)