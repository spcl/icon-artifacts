...
MODULE mo_fortran_tools
  IMPLICIT NONE
  CONTAINS
  PURE SUBROUTINE set_acc_host_or_device(global_data, lzacc, lacc)
...
MODULE mo_lib_loopindices
  IMPLICIT NONE
  CONTAINS
  SUBROUTINE get_indices_c_lib(global_data, i_startidx_in, i_endidx_in, nproma, i_blk, i_startblk, i_endblk, i_startidx_out, i_endidx_out)
    USE global_mod, ONLY: global_data_type
    TYPE(global_data_type) :: global_data
  ...
  SUBROUTINE get_indices_e_lib(global_data, i_startidx_in, i_endidx_in, nproma, i_blk, i_startblk, i_endblk, i_startidx_out, i_endidx_out)
    USE global_mod, ONLY: global_data_type
    TYPE(global_data_type) :: global_data
    INTEGER, INTENT(IN) :: i_startidx_in
    INTEGER, INTENT(IN) :: i_endidx_in
  ...
  SUBROUTINE get_indices_v_lib(global_data, i_startidx_in, i_endidx_in, nproma, i_blk, i_startblk, i_endblk, i_startidx_out, i_endidx_out)
    USE global_mod, ONLY: global_data_type
    TYPE(global_data_type) :: global_data
    INTEGER, INTENT(IN) :: i_startidx_in
    INTEGER, INTENT(IN) :: i_endidx_in
...
MODULE mo_lib_interpolation_scalar
  IMPLICIT NONE
  CONTAINS
  SUBROUTINE cells2verts_scalar_ri_lib(global_data, p_cell_in, vert_cell_idx, vert_cell_blk, c_int, p_vert_out, i_startblk, i_endblk, i_startidx_in, i_endidx_in, slev, elev, nproma, lacc, acc_async)
    USE global_mod, ONLY: global_data_type
    ...
    LOGICAL :: lzacc
    CALL set_acc_host_or_device(global_data, lzacc, lacc)
    DO jb = i_startblk, i_endblk
      CALL get_indices_v_lib(global_data, i_startidx_in, i_endidx_in, nproma, jb, i_startblk, i_endblk, i_startidx, i_endidx)
      DO jk = 1, elev
...
MODULE mo_loopindices
  IMPLICIT NONE
  CONTAINS
  SUBROUTINE get_indices_c(global_data, p_patch, i_blk, i_startblk, i_endblk, i_startidx, i_endidx, irl_start, opt_rl_end)
    USE global_mod, ONLY: global_data_type
    ...
    CALL get_indices_c_lib(global_data, i_startidx_in, i_endidx_in, global_data % nproma, i_blk, i_startblk, i_endblk, i_startidx, i_endidx)
  END SUBROUTINE get_indices_c
  SUBROUTINE get_indices_e(global_data, p_patch, i_blk, i_startblk, i_endblk, i_startidx, i_endidx, irl_start, opt_rl_end)
    USE global_mod, ONLY: global_data_type
    USE mo_model_domain, ONLY: t_patch
    ...
    CALL get_indices_e_lib(global_data, i_startidx_in, i_endidx_in, global_data % nproma, i_blk, i_startblk, i_endblk, i_startidx, i_endidx)
  END SUBROUTINE get_indices_e
  SUBROUTINE get_indices_v(global_data, p_patch, i_blk, i_startblk, i_endblk, i_startidx, i_endidx, irl_start, opt_rl_end)
    USE global_mod, ONLY: global_data_type
    USE mo_model_domain, ONLY: t_patch
    USE mo_lib_loopindices, ONLY: get_indices_v_lib
    TYPE(global_data_type) :: global_data
    ...
    CALL get_indices_v_lib(global_data, i_startidx_in, i_endidx_in, global_data % nproma, i_blk, i_startblk, i_endblk, i_startidx, i_endidx)
  END SUBROUTINE get_indices_v
END MODULE mo_loopindices
MODULE mo_math_divrot
  IMPLICIT NONE
  CONTAINS
  SUBROUTINE rot_vertex_ri(global_data, vec_e, ptr_patch, ptr_int, rot_vec, opt_slev, opt_elev, opt_rlend, opt_acc_async)
    USE global_mod, ONLY: global_data_type
    USE mo_model_domain, ONLY: t_patch
    ...
    DO jb = i_startblk, i_endblk
      CALL get_indices_v(global_data, ptr_patch, jb, i_startblk, i_endblk, i_startidx, i_endidx, 2, - 5)
      DO jk = slev, elev
        DO jv = i_startidx, i_endidx
...
MODULE mo_real_timer
  IMPLICIT NONE
  CONTAINS
  SUBROUTINE timer_start(global_data, it)
    USE global_mod, ONLY: global_data_type
    TYPE(global_data_type) :: global_data
    INTEGER, INTENT(IN) :: it
  END SUBROUTINE timer_start
  SUBROUTINE timer_stop(global_data, it)
    USE global_mod, ONLY: global_data_type
    TYPE(global_data_type) :: global_data
    INTEGER, INTENT(IN) :: it
...
MODULE mo_icon_interpolation_scalar
  IMPLICIT NONE
  CONTAINS
  SUBROUTINE cells2verts_scalar_ri(global_data, p_cell_in, ptr_patch, c_int, p_vert_out, opt_slev, opt_elev, opt_rlstart, opt_rlend, opt_acc_async)
    USE global_mod, ONLY: global_data_type
    USE mo_model_domain, ONLY: t_patch
    ...
    IF (global_data % timers_level > 10) CALL timer_start(global_data, global_data % timer_intp)
    CALL cells2verts_scalar_ri_lib(global_data, p_cell_in, ptr_patch % verts % cell_idx, ptr_patch % verts % cell_blk, c_int, p_vert_out, i_startblk, i_endblk, i_startidx_in, i_endidx_in, 1, elev, global_data % nproma, lacc = global_data % i_am_accel_node, acc_async = .TRUE.)
    IF (global_data % timers_level > 10) CALL timer_stop(global_data, global_data % timer_intp)
...
MODULE mo_velocity_advection
  ...
  SUBROUTINE velocity_tendencies(global_data, p_prog, p_patch, p_int, p_metrics, p_diag, z_w_concorr_me, z_kin_hor_e, z_vt_ie, ntnd, istep, lvn_only, dtime, dt_linintp_ubc, ldeepatmo)
    ...
    END IF
    IF (.NOT. lvn_only) CALL cells2verts_scalar_ri(global_data, p_prog % w, p_patch, p_int % cells_aw_verts, z_w_v, opt_rlend = - 5, opt_acc_async = .TRUE.)
    CALL rot_vertex_ri(global_data, p_prog % vn, p_patch, p_int, zeta, opt_rlend = - 5, opt_acc_async = .TRUE.)
    IF (istep == 1) THEN
      rl_start = 5
      rl_end = -10
      ...
      DO jb = i_startblk, i_endblk
        CALL get_indices_e(global_data, p_patch, jb, i_startblk, i_endblk, i_startidx, i_endidx, 5, - 10)
        DO jk = 1, nlev
    ...
    IF (.NOT. lvn_only) THEN
      DO jb = i_startblk, i_endblk
        CALL get_indices_e(global_data, p_patch, jb, i_startblk, i_endblk, i_startidx, i_endidx, 7, - 9)
        DO jk = 1, nlev
          DO je = i_startidx, i_endidx
    ...
    DO jb = i_startblk, i_endblk
      CALL get_indices_c(global_data, p_patch, jb, i_startblk, i_endblk, i_startidx, i_endidx, 4, - 5)
      DO jk = 1, nlev
      ...
      CALL get_indices_c(global_data, p_patch, jb, i_startblk_2, i_endblk_2, i_startidx_2, i_endidx_2, 5, - 4)
      DO jk = 2, nlev
    ...
    DO jb = i_startblk, i_endblk
      CALL get_indices_e(global_data, p_patch, jb, i_startblk, i_endblk, i_startidx, i_endidx, 10, - 8)
    ...
    max_vcfl_dyn = MAX(0.0D0, MAXVAL(vcflmax(i_startblk : i_endblk)))
