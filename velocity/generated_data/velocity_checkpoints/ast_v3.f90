...
MODULE mo_velocity_advection
  ...
  SUBROUTINE velocity_tendencies(p_prog, p_patch, p_int, p_metrics, p_diag, z_w_concorr_me, z_kin_hor_e, z_vt_ie, ntnd, istep, lvn_only, dtime, dt_linintp_ubc, ldeepatmo)
    ...
    REAL(KIND = 8) :: z_w_concorr_mc(nproma, 90)
    REAL(KIND = 8) :: z_w_con_c(nproma, 91)
    REAL(KIND = 8) :: z_w_con_c_full(nproma, 90, p_patch % nblks_c)
    REAL(KIND = 8) :: z_v_grad_w(nproma, 90, p_patch % nblks_e)
    REAL(KIND = 8) :: z_w_v(nproma, 91, p_patch % nblks_v)
    REAL(KIND = 8) :: zeta(nproma, 90, p_patch % nblks_v)
    REAL(KIND = 8) :: z_ekinh(nproma, 90, p_patch % nblks_c)
    INTEGER :: nlev, nlevp1
